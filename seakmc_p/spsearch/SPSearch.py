...
class SPSearch():
    ...
    def reprepare_runner(self):
        ...
        [total_energy, coords, isValid, errormsg] = self.force_evaluator.init_spsearch_runner(thisdata, self.thiscolor, self.nactive, comm = self.comm)
    ...
    def calculate_prefactor(self):
        if self.ISVALID and self.dmAV is not None:
            ...
            [encalc, coords, isValid, errormsg] = self.force_evaluator.run_runner("SPSDYNMAT", thisdata, self.thiscolor, nactive = self.data.nactive, comm = self.comm)
            if not isValid and self.rank_this == 0:
                print(errormsg)
                comm_world.Abort(rank_world)
            thisdata = None
...
class Dimer(SPSearch):
    ...
    def dimer_init(self, thisVN=None):
        ...
        self.iter = 0
        [total_energy, coords, isValid, errormsg]=self.force_evaluator.init_spsearch_runner(self.data, self.thiscolor, self.nactive, comm = self.comm)
        if not isValid and self.rank_this == 0:
            print(errormsg)
            comm_world.Abort(rank_world)
        self.comm.Barrier()
    ...
    def dimer_force(self):
        ...
        [encalc, FC, isValid, errormsg] = self.force_evaluator.get_spsearch_forces(self.XA.T, self.data, self.thiscolor, self.nactive, comm=self.comm)
        if not isValid and self.rank_this == 0:
            print(errormsg)
            comm_world.Abort(rank_world)
        self.comm.Barrier()
    ...
    def dimer_relaxation(self):
        ...
        for i in range(len(X12)):
            ...
            self.force_evaluator.close()
            [encalc, coords, isValid, errormsg] = self.force_evaluator.run_runner("SPSRELAX", thisdata, self.thiscolor, nactive = self.nactive, comm=self.comm)
            if not isValid and self.rank_this == 0:
                print(errormsg)
                comm_world.Abort(rank_world)
            mask = np.ones(3*self.nactive, dtype = bool)
