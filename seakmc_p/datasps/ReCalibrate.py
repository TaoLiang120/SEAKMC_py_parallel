...
def recalib_energy_single(itask_start, thiscolor, comm_split, \
    ...
    if isinstance(thissett.saddle_point["Thres4Recalib"], float) or isinstance(thissett.saddle_point["Thres4Recalib"], int):
        if DataSPs.df_SPs[iav].at[lisp, "barrier"] - DataSPs.barriermin > thissett.saddle_point["Thres4Recalib"]: thisRecal = False
    ...
    if thisRecal:
        ...
        [thisesp, coords, isValid, errormsg] = force_evaluator.run_runner("MD0", thisdata, thiscolor, nactive = thisdata.natoms, thisExports = None, comm = comm_split)
        ...
        if ReBias:
            ...
            [thisefin, coords, isValid, errormsg] = force_evaluator.run_runner("MD0", thisdata, thiscolor, nactive = thisdata.natoms, thisExports = None, comm = comm_split)
            if not isValid and rank_local==0:
                print(errormsg)
                comm_world.Abort(rank_world)
