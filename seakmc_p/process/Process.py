...
from seakmc_p.input.Input import SP_COMPACT_HEADER, SP_COMPACT_HEADER4Delete, SP_DATA_HEADER
from seakmc_p.core.data import SeakmcData
from seakmc_p.restart.Restart import RESTART
from seakmc_p.kmc.KMC import Basin, SuperBasin
...
def run_seakmc(thissett, seakmcdata, object_dict, Eground, thisRestart):
    ...
    if thisRestart is None:
        ...
        if thissett.defect_bank["LoadDB"]:
            DefectBank_list = preSPS.load_DefectBanks(thissett.defect_bank, out_paths[2], significant_figures=thissett.system["significant_figures"])
        istep_this = 0
    else:
    ...
    for istep in range(istep_this, thissett.kinetic_MC['NSteps']):
        ...
        DataSPs, AVitags, df_delete_SPs = dataSPS.data_find_saddlepoints(istep, thissett, seakmcdata, DefectBank_list, thisSuperBasin, Eground, \
        ...
        if thissett.saddle_point["CalBarrsInData"]:
            DataSPs, df_delete_this = myRecal.recalibrate_energy(thissett, DataSPs, seakmcdata, AVitags, Eground, object_dict)
            if rank_world==0:
        ...
        if len(sel_SPs)>0: dataout.visualize_data_SPs(thissett.visual, seakmcdata, AVitags, DataSPs, sel_SPs, istep, out_paths[1])
            ...
            if rank_world==0: dataout.visualize_data_SPs_Superbasin(thissett.visual, thiskmc, thisSuperBasin, istep, out_paths[1])
        ...
        [Eground, relaxed_coords, isValid, errormsg] = mydatadyn.data_dynamics("OPT", force_evaluator, seakmcdata, 1, \
                        nactive = seakmcdata.natoms, nproc_task = nproc_task, thisExports=thisExports)
