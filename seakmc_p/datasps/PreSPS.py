...
from seakmc_p.core.symmetry import PGSymmOps, SGSymmOps, SymmOP
from seakmc_p.dynmat.Dynmat import DynMat, VibMat, VibMats
from seakmc_p.spsearch.SaddlePoints import AV_SPs, Data_SPs, DefectBank
...
def get_AV_atom_strain(thisAV, thissett, thiscolor, comm=None):
    if thisAV is not None:
        if "LAS" in thissett.spsearch["HandleVN"]["RescaleValue"].upper() and thissett.spsearch["HandleVN"]["RescaleVN"]:
            thisStrain = thisAV.estimate_atom_strain(thissett, nactive=thisAV.nactive, nbuffer=thisAV.nbuffer, comm=comm)
...
def get_dynmatAV(idav, thissett, thisAV, force_evaluator, thiscolor, comm=None):
    [etotal, coords, isValid, errormsg] = force_evaluator.run_runner("SPSDYNMAT", thisAV, thiscolor, \
            nactive = thisAV.nactive, thisExports = None, comm=comm)
