...
def initial_data_dynamics(thissett, seakmcdata, force_evaluator, LogWriter):
    ...
    if thissett.data["MoleDyn"]:
        ...
        comm_world.Barrier()
        [Eground, relaxed_coords, isValid, errormsg]= mydatadyn.data_dynamics("DATAMD", force_evaluator, seakmcdata, ntask_tot, \
                nactive = seakmcdata.natoms, nproc_task = nproc_task, thisExports=None)
        ...
        [Eground, relaxed_coords, isValid, errormsg] = mydatadyn.data_dynamics("DATAOPT", force_evaluator, seakmcdata, ntask_tot, \
                nactive = seakmcdata.natoms, nproc_task = nproc_task, thisExports=None)
        ...
        [Eground, relaxed_coords, isValid, errormsg] = mydatadyn.data_dynamics("DATAMD0", force_evaluator, seakmcdata, ntask_tot, \
                nactive = seakmcdata.natoms, nproc_task = nproc_task, thisExports=None)
...
def preprocess(thissett):
    ...
    if thisRestart is None:
        ...
        if Eground is None or Eground == 0.0:
            [Eground, relaxed_coords, isValid, errormsg] = mydatadyn.data_dynamics("DATAMD0", force_evaluator, seakmcdata, 1, \
                nactive = seakmcdata.natoms, nproc_task = thissett.force_evaluator['nproc'], thisExports=None)
