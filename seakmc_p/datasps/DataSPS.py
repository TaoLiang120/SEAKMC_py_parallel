...
from seakmc_p.spsearch.SaddlePoints import check_duplicate_avSPs
...
def AV_is_done(idav, iav, idsps, ticav_l, thisAV_l, AVstring_l, thisSPS_l, \
    ...
    ticav = ticav_l[iav]
    thisSPS_l[iav], df_delete_SPs  = postSPS.SPs_1postprocessing(thissett, thisSPS_l[iav], df_delete_SPs, DFWriter, nSPstart=thisSPS_l[iav].nSP)
    thisSPS_l[iav], df_delete_this = DataSPs.check_dup_avSP(idav, thisSPS_l[iav], seakmcdata.de_neighbors, AVitags, thissett.saddle_point)
    ...
    if thisSPS_l[iav].nSP > 0:
        ...
        if thissett.defect_bank["Recycle"]:
            DefectBank_list = postSPS.add_to_DefectBank(thissett, thisAV_l[iav], thisSPS_l[iav], isRecycled_l[iav], \
                      isPGSYMM_l[iav], thisSOPs_l[iav].sch_symbol, DefectBank_list, object_dict['out_paths'][3])
        DataSPs = postSPS.insert_AVSP2DataSPs(DataSPs, thisSPS_l[iav], idav, DFWriter)
        dataout.visualize_AV_SPs(thissett.visual, seakmcdata, AVitags, thisAV_l[iav], thisSPS_l[iav], istep, idav, object_dict['out_paths'][0])

        AVstring_l[iav] += "\n" + f"Found {str(thisSPS_l[iav].nSP)} saddle points in {str(idav)} active volume!"
        AVstring_l[iav] += "\n"+"-----------------------------------------------------------------"

    tocav = time.time()
    AVstring_l[iav] += "\n"+f"Total time for {idav} active volume: {round(tocav-ticav_l[iav], thissett.system['float_precision'])} s"
    AVstring_l[iav] += "\n"+"-----------------------------------------------------------------"
    ...
    while completed_tasks<ntask_tot:
        ...
        if thiscolor in working_jobs:
            if tag == MPI_Tags.DONE:
                    ...
                    if thisSP.ISVALID:
                        ...
                        if thisSP.ISVALID:
                            if thisSOPs_l[iav].nOP > 1:
                                thisSymmsps = thisSP.get_SPs_from_symmetry(thisAV_l[iav].to_coords(Buffer=False, Fixed=False), thisSOPs_l[iav])
                ...
                AVstring_l[iav] += "\n" + f"idAV:{idav}, idsps:{idsps},  ntrans: {ntsiter},  barrier:{round(thisSP.barrier,float_precision)},  ebias: {round(thisSP.ebias,float_precision)}"
                AVstring_l[iav] += "\n" + f"      dmag:{round(thisSP.dmag,float_precision)}, dmagFin:{round(thisSP.fdmag,float_precision)}, isConnect: {thisSP.isconnect}"
                AVstring_l[iav] += "\n" + f"      Valid SPs: {thisSPS_l[iav].nvalid}, num of SPs:{thisSPS_l[iav].nSP}, time: {round(time_spsearch, float_precision)} s"
                AVstring_l[iav] += "\n" + "-----------------------------------------------------------------"

                isVisit[iav] = True
                completed_spsearchs_AV[iav] +=1
                if completed_spsearchs_AV[iav] == thisnspsearch: isAVDone = True
                else: isAVDone = False
                ...
                if task_index<ntask_tot:
                    ...
                    if ticav_l[iav] is None:
                        ...
                        if thisAV is not None:
                            ...
                            AVstring_l[iav] += "\n" + f"ActVol ID: {idav} nactive:{thisAV.nactive} nbuffer:{thisAV.nbuffer} nfixed:{thisAV.nfixed}"
                            AVstring_l[iav] += "\n" + f"AV center fractional coords: {round(seakmcdata.defects.at[idav,'xsn'],float_precision), round(seakmcdata.defects.at[idav,'ysn'],float_precision), round(seakmcdata.defects.at[idav,'zsn'],float_precision)}"
                            ...
                            AVstring_l[iav] += "\n"+"-----------------------------------------------------------------"
                            thisSPS_l[iav] = preSPS.initialize_thisSPS(idav, np.array([[],[],[]]), thisnspsearch, thissett)
                            SNC_l[iav] = False
                            CalPref_l[iav] = False
            ...
            elif tag == MPI_Tags.EXIT: completed_tasks += 1
        ...
        if tag == MPI_Tags.START:
            ...
            if thisAV is not None:
                if not isVisit_this:
                    ...
                    dynmatAV_l[iav] = None
                    if SNC or CalPref:
                        SNC, CalPref, dynmatAV_l[iav], isDynmat = preSPS.get_thisSNC4spsearch(idav, thissett, thisAV, SNC, CalPref, object_dict, thiscolor, comm=comm_split)
                        comm_split.Barrier()
                else:
                ...
                thisSP, thisVN, time_spsearch, ntsiter = mySPS.spsearch_search_single(nproc_task, thiscolor, comm_split, \
                                idav, thissett, thisAV, thisSOPs, dynmatAV_l[iav], SNC, CalPref, thisSPS, \
        ...
        if thiscolor < ntask_time:
            ...
            if not isVisit[iav]:
                ...
                if thisAV is not None:
                    if rank_local==0:
                        AVitags[idav] = thisAV.itags[0:(thisAV.nactive+thisAV.nbuffer)]
                        AVstring += f"ActVol ID: {idav} nactive:{thisAV.nactive} nbuffer:{thisAV.nbuffer} nfixed:{thisAV.nfixed}"
                        AVstring += "\n" + f"AV center fractional coords: {round(seakmcdata.defects.at[idav,'xsn'],float_precision), round(seakmcdata.defects.at[idav,'ysn'],float_precision), round(seakmcdata.defects.at[idav,'zsn'],float_precision)}"
                    ...
                    if rank_local==0: OptResults =[thisAV, AVstring, ticav, thisSPS, local_coords, Pre_Disps, \
                    ...
                    if rank_local==0: OptResults =[None, AVstring, ticav, thisSPS, local_coords, [], None, None, None, False, False, False, np.array([], dtype=int)]
                    else: OptResults=[]

                if thiscolor>0 and rank_local==0:
                    Send_Results(thiscolor, iav, idav, idsps, thisSP, thisVN, time_spsearch, ntsiter, isSend[thiscolor], comm_world, *OptResults)
            else:
                if thisAV_l[iav] is not None:
                    if isDynmat_l[iav] and dynmatAV_l[iav] is None:
                        SNC_l[iav], CalPref_l[iav], dynmatAV_l[iav], isDynmat_l[iav] = preSPS.get_thisSNC4spsearch(idav, thissett, \
                                                        thisAV_l[iav], SNC_l[iav], CalPref_l[iav], object_dict, thiscolor, comm=comm_split)
                        comm_split.Barrier()

                    thisSP, thisVN, time_spsearch, ntsiter = mySPS.spsearch_search_single(nproc_task, thiscolor, comm_split, \
                                idav, thissett, thisAV_l[iav], thisSOPs_l[iav], dynmatAV_l[iav], SNC_l[iav], CalPref_l[iav], thisSPS_l[iav], \
                                Pre_Disps_l[iav], idsps, thisVNS_l[iav], \
                                object_dict)
                ...
                if thiscolor>0 and rank_local==0:
                    Send_Results(thiscolor, iav, idav, idsps, thisSP, thisVN, time_spsearch, ntsiter, False, comm_world, *OptResults)
        else:
            thisSP = None
            thisVN = None
            time_spsearch = None
        ...
        if rank_world==0:
            ...
            if not isVisit[iav]:
               ...
               if thisAV_l[iav] is not None:
                   ...
                   if len(AVstring_head_l[iav])>10: pass
                   else: AVstring_head_l[iav] += AVstring
...
def data_find_saddlepoints(istep, thissett, seakmcdata, DefectBank_list, thisSuperBasin, Eground, \
    ...
    if Master_Slave:
        ...
        DataSPs, AVitags, df_delete_SPs = data_SPS_no_master_slave(nproc_task, start_proc, ntask_time, thiscolor, comm_split, \
               istep, thissett, seakmcdata, DefectBank_list, thisSuperBasin, Eground, \
               DataSPs, AVitags, df_delete_SPs, undo_idavs, finished_AVs, simulation_time, object_dict)
